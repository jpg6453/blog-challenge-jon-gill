...
app.get("/blogs/:slug", (req, res) => {
    ...
    .then((result) => {
        ...
        if(postIndex < 0) {
            ...
            axios.get(hostname + '/posts/'+ data.blog.id + '/comments')
            .then((result) => {
            ...
            .then(() => {
                ...
                res.render('blog-detail', {data: data, title: data.blog.title, error});
...
app.post("/newcomment", [
    check('name', 'Please enter your name')
        .isLength({min:1})
        .trim(),
    check('content', 'Please write a comment')
        .isLength({min:1})
        .trim()
], (req, res, next) => {
    ...
    // POST to api
    axios.post(hostname + '/posts/' + postId + '/comments/', comment, errorMsg)
